...
<body>	  
	...
	<% ArrayList<BestDTO> BL = (ArrayList<BestDTO>) request.getAttribute("BL"); %>
			  ...
			  <div class="d-flex flex-column text-center">
				<form action="mem_login" method="post">
				  <div class="form-group">
				   <input type="hidden" name = "url" value="best_show">
	...
	outline: 0;" ><i class="login">logout</i></button>
			...
			<nav class="main__menu2">
				<ul class="nav__menu2">
					...
					<li><a href="./Nutrients.html">영양제</a> -->
				</ul>
			</nav>
		</div>
	</header>
...
<%-- 	<div class="supplements_container">
	...
	<div class="col-lg-2 col-md-4 col-sm-3" id ="list1" style="width: 18rem;">
		...
		if(mdto.getSup_seq()==BL.get(i).getSup_seq()) 
		{ 
		...
		<input type="hidden" name="seq" value=<%=BL.get(i).getSup_seq()%>>
		<input type="hidden" name="id" value=<%=dto.getMem_id()%>>
		<input type="hidden" name="url" value="best_show">
		<button type="submit"><img alt="" src=<%=src %>></button>
		</form>
		<%}%>
		<a href=<%=BL.get(i).getSup_link() %> class="fa fa-bookmark pull-right">
		<img src=<%=BL.get(i).getSup_img() %> class="card-img-top" alt="...">
		</a>		
		<div class="card-body">
		  <h5 class="card-title"><%= BL.get(i).getSup_name() %></h5>
		  <p class="card-text" style="text-align: center;"><%= BL.get(i).getSup_price()%>원</p>
		  <p class="card-text"><%= BL.get(i).getSup_description()%></p>
		  <p class="card-text"><%= BL.get(i).getSup_func() %></p>
		</div>
	  </div>
	<%}%>
	</div>
		...
		<div class="row">
			...
			<div class="col-lg-2 col-md-4 col-sm-3" id="list1"
				...
				<form action=<%=action%> style="text-align: right">
					...
					<input type="hidden" name="id" value=<%=dto.getMem_id()%>>
					<input type="hidden" name="url" value="best_show">
					<button type="submit" style="width: 25%; height: 25%; background: none;
	border: 0;
	outline: 0;" >
						<img alt="" src=<%=src%>>
					</button>
				...
				%>
				<a href=<%=BL.get(i).getSup_link()%>
					> <img
					src=<%=BL.get(i).getSup_img()%> class="card-img-top" alt="...">
				</a>
				<div class="card-body">
					<h5 class="card-title"><%=BL.get(i).getSup_name()%></h5>
					<p class="card-text" style="text-align: center;"><%=BL.get(i).getSup_price()%>원
					</p>
					<p class="card-text"><%=BL.get(i).getSup_description()%></p>
					<p class="card-text"><%=BL.get(i).getSup_func()%></p>
				</div>
